...
on)
	...
	LCK="/usr/local/nginx/scripts/tmp/${ffscreen}.LCK"
	exec 8>$LCK
	...
	if flock -n -x 8; then
		if [ -z "$STY" ]; then
			...
			#sleep .2
			exec screen -dm -S $ffscreen /bin/bash "$0" "$1" "$2" "$3"
		fi

		while true; do
			ffmpeg -i rtmp://127.0.0.1/$streamapp/$streamid -f mpegts -acodec mp2 -ar 44100 -ac 1 -b:a 96k -vcodec mpeg1video -r 25 -bf 0 -s 320x180 -b:v 200k -muxdelay 0.001 http://localhost:8031/supersecret
