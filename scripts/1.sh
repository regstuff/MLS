...
while true #This infinite loop will run inside the screen
do
...
while true #Send main input to pipe
do
...
while true #Playlist works by concating files from list.txt
do
...
while [ $i -lt 9000 ] #Turn off if output is idle for 9000*0.2=1800 seconds
do #Use transpose filter to transpose 90 degrees.
